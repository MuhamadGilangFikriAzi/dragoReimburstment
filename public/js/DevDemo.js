...
function sendData()
{
	...
	var pasien_nama 		= $('#pasien_nama').val();
	var pasien_nik  		= $('#pasien_no').val();
	var lab_order			= $('#order_no').val(); 
	var perusahaan_kode		= $('#perusahaan').val();
	var qty 				= $('#jumlah_barcode').val();
	var kode_paket 			= $('#kode_paket').val();
	var format_start 		= "^XA^PQ"+qty+"^FO10,10^ADN,20,12^FD"+pasien_nama+"^FS"
							 	+"^FO10,30^ADN,20,12^FD"+pasien_nik+"/"+kode_paket+" - "+perusahaan_kode+""
						 		+"^FS^FO30,50^BY2^BCN,70,Y,N,N"
						 		+"^ADN,28,14^FD"+lab_order+"^FS^XZ";
								// +"^FS^XZ";
	// var format_start 		= "^XA"
	// 							+"^F0100,75"
	// 							+"^BCN,100,Y,N,N"
...
function sendDataSingle()
{
	...
	var perusahaan_kode		= $('#perusahaan').val();
	var kode_paket 			= $('#kode_paket').val();
	var format_start 		= "^XA^FO10,10^ADN,20,12^FD"+pasien_nama+"^FS"
							 	+"^FO10,30^ADN,20,12^FD"+pasien_nik+"/"+kode_paket+" - "+perusahaan_kode+""
						 		+"^FS^FO30,50^BY2^BCN,70,Y,N,N"
						 		+"^ADN,28,14^FD"+lab_order+"^FS^XZ";
								// +"^FS^XZ";
